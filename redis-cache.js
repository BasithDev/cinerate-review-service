...
class RedisCache {
  constructor(options = {}) {
    ...
    this.prefix = options.prefix || 'review-service:';
    this.client = null;
  ...
  cacheMiddleware(ttl = this.ttl) {
    return async (req, res, next) => {
      ...
      // For movie/show reviews, we can cache by movie ID
  ...
  // Helper to invalidate movie-specific cache
  async invalidateMovieCache(movieId) {
    return this.invalidateByPattern(`*movie=${movieId}*`);
  }

  // Helper to invalidate user's reviews cache
  async invalidateUserReviewsCache(userId) {
